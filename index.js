...
function startBot(message) {
    ...
    const child = spawn("node", ["--trace-warnings", "--async-stack-traces", "Neha.js"], {
        cwd: __dirname,
        stdio: "inherit",
        shell: true
    });
    ...
    child.on("close", (codeExit) => {
        if (codeExit !== 0) {
            ...
            if (global.countRestart < 5) {
                ...
                logger("To see detailed errors, check the logs above or run the bot with 'node Neha.js' directly", "[ Debug ]");
            }
        } else {

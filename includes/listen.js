module.exports = function ({ api, models }) {
    ...
    const checkttDataPath = __dirname + '/../Neha/commands/checktuongtac/';
    setInterval(async () => {
            ...
            await new Promise(async resolve => {
                for (const checkttFile of checkttData) {
                    ...
                    for (const item of checktt.day) {
                        const userName = await Users.getNameUser(item.id) || 'Neha Thakur';
    ...
    (async function () {
        ...
        try {
            logger(global.getText('listen', 'startLoadEnvironment'), '[ Neha  Thakur ]');
            ...
            logger.loader(global.getText('listen', 'loadedEnvironmentUser')), logger(global.getText('listen', 'successLoadEnvironment'), '[ Neha ]');
        } catch (error) {
            return logger.loader(global.getText('listen', 'failLoadEnvironment', error), 'error');
        }
    }());
    logger(`[ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "" : global.config.BOTNAME}`, "[ Neha Thakur ]");
    ...
    //DEFINE DATLICH PATH
    const datlichPath = __dirname + "/../Neha/commands/cache/datlich.json";
        ...
        for (el of temp) {
            ...
            if ("ATTACHMENT" in el) {
                ...
                for (a of el.ATTACHMENT) {
                    ...
                    fs.writeFileSync(__dirname + `/../Neha/commands/cache/${a.fileName}`, Buffer.from(getAttachment, 'utf-8'));
                    out.attachment.push(fs.createReadStream(__dirname + `/../Neha/commands/cache/${a.fileName}`));
                }
            }
            console.log(out);
            if ("BOX" in el) await api.setTitle(el["BOX"], el["TID"]);
            api.sendMessage(out, el["TID"], () => ("ATTACHMENT" in el) ? el.ATTACHMENT.forEach(a => fs.unlinkSync(__dirname + `/../Neha/commands/cache/${a.fileName}`)) : "");
        }
